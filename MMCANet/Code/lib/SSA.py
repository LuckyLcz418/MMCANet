...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        '''
            sr_ratio:空间降维的缩减比例, 如果大于1, 则进行降维;
        ...
        if sr_ratio > 1:
            ...
            # 创建了一个分组卷积层。分组卷积是指将通道分成多个组，每组通道之间独立进行卷积操作，然后将计算结果进行合并
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # (B, num_heads, N, C_num_head)
        if self.sr_ratio > 1:  # 降维
                x_ = x.permute(0, 2, 1).reshape(B, C, H, W)  # (B, C, H, W)
                # 假设sr_ratio=8
                # 变化过程：(B, C, H, W) -> (B, C, H/8, W/8) -> (B, C, HW/64) -> (B, HW/64, C)
                x_1 = self.act(self.norm1(self.sr1(x_).reshape(B, C, -1).permute(0, 2, 1)))  # 对x进行两次不同的降维得到x_1，x_2
                # 变化过程：(B, C, H, W) -> (B, C, H/4, W/4) -> (B, C, HW/16) -> (B, HW/16, C)
                ...
                k1, v1 = kv1[0], kv1[1] # (B, num_heads//2, HW/64, C_num_head)
                k2, v2 = kv2[0], kv2[1]
                attn1 = (q[:, :self.num_heads//2] @ k1.transpose(-2, -1)) * self.scale # (B, num_heads//2, HW/64, C_num_head, HW, HW/64)
...
if __name__ == "__main__":
    ...
    m = shunted_t(pretrained='/media/data2/lcl_e/lcz/lcz/RGBT_scribble/checkpoints/ckpt_S.pth')
    y = m(x)
    print(len(y))
    print(y[0].shape)
    print(y[1].shape)
